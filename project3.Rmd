...
## Linear Regression  
We fit two different linear regression models here. This is a basic method to find a linear relationship between a response variable and one or more prediction variables. Here we will fit models using a forward selection and subset selection of quadratic and interaction terms.
...
Here we fit a random forest model.
...
rand_fit <- train(shares~.,
                  ...
                  trControl = trainControl(method = "repeatedcv", number = 5, repeats = 3, p=0.7),
                  tuneGrid = rand_grid)
rand_fit$bestTune
rand_fit$finalModel
...
Below is the process of fitting a boosted tree model, also model performance is tested by fitting the final tuned model on test set and calculate the test RMSE. Here, I use the 
